...
* $W=V\Lambda V^\top$  
  ...
  $V^\top$ is just the transpose which turns rows into columns and vice versa  
...
* $\Lambda=\mathrm{diag}(\lambda_1\ \lambda_2\ \dots\ \lambda_n)$  
  ...
  Bigger $\lambda_k$ means that mode contributes more strongly to the system’s behavior
...
* "So the spectral gap decreases during unlearning"  
